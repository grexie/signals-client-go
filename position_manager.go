...
// RemoveInstrumentConfig removes fee and leverage overrides for one
// venue/instrument pair after the host has removed it from the active universe.
func (pm *PositionManager) RemoveInstrumentConfig(venue, instrument string) {
	if pm == nil || venue == "" || instrument == "" {
		return
	}
	pm.mu.Lock()
	...
	delete(pm.cfg.Instruments, positionKey(venue, instrument))
}
